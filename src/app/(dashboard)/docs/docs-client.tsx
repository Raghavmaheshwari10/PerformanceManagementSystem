...
import { CHANGELOG, type RoleTag } from './changelog'

// ─── Section data ──────────────────────────────────────────────────────────────

const SECTIONS = [
  { id: 'whats-new',      label: "What's New",            roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
  { id: 'overview',        label: 'Overview',              roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
  { id: 'roles',           label: 'User Roles',            roles: ['admin', 'hrbp'] as RoleTag[] },
  { id: 'cycle-lifecycle', label: 'Cycle Lifecycle',       roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
  { id: 'kra-kpi',         label: 'KRA & KPI Setup',       roles: ['employee', 'manager', 'admin'] as RoleTag[] },
  { id: 'goals',           label: 'Goals',                 roles: ['employee', 'manager'] as RoleTag[] },
  { id: 'competency',      label: 'Competency Assessment', roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
  { id: 'scoring',         label: 'Score Engine',          roles: ['admin', 'hrbp', 'manager'] as RoleTag[] },
  { id: 'mis',             label: 'MIS Integration',       roles: ['admin', 'hrbp', 'manager'] as RoleTag[] },
  { id: 'feedback',        label: 'Feedback',              roles: ['employee', 'manager'] as RoleTag[] },
  { id: 'user-flows',      label: 'User Flows',            roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
  { id: 'calculations',    label: 'Payout Calculations',   roles: ['admin', 'hrbp'] as RoleTag[] },
  { id: 'admin-config',    label: 'Admin Configuration',   roles: ['admin'] as RoleTag[] },
  { id: 'quick-ref',       label: 'Quick Reference',       roles: ['employee', 'manager', 'hrbp', 'admin'] as RoleTag[] },
]
...
export default function DocsClient({ userRole }: { userRole: UserRole }) {
  const [activeId, setActiveId] = useState('whats-new')
  const [filteredRole, setFilteredRole] = useState<RoleTag>(userRole as RoleTag)
      ...
      <aside className="hidden lg:flex flex-col w-48 flex-shrink-0">
        <div className="sticky top-0 pt-1">
          ...
          <nav className="flex flex-col gap-0.5">
            {SECTIONS.map(s => {
              const relevant = filteredRole === 'all' || s.roles.includes(filteredRole)
              return (
                <a
                  key={s.id}
                  href={`#${s.id}`}
                ...
                >
                  {s.label}
                  {!relevant && <span className="ml-1 text-[10px]">—</span>}
                </a>
              )
            })}
          </nav>
      ...
      <div ref={contentRef} className="flex-1 max-w-3xl space-y-10 pb-16 min-w-0">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['employee', 'manager', 'hrbp', 'admin'] as RoleTag[]).map(role => (
              <button
                key={role}
                ...
                className={cn(
                  ...
                  filteredRole === role
                    ? role === 'admin'   ? 'bg-blue-600 text-white border-blue-600'
                    : role === 'manager' ? 'bg-purple-600 text-white border-purple-600'
                    : role === 'hrbp'    ? 'bg-amber-500 text-white border-amber-500'
                    :                     'bg-green-600 text-white border-green-600'
                    : 'bg-background text-muted-foreground hover:text-foreground border-border'
                )}
              >
                {role === 'hrbp' ? 'HRBP' : role.charAt(0).toUpperCase() + role.slice(1)}
                {role === userRole && <span className="ml-1 opacity-70">(you)</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-4">Recent features and improvements — filtered to changes relevant for your role.</p>
          <div className="space-y-6">
            {CHANGELOG.map((entry, ei) => {
              const relevant = entry.changes.filter(c =>
                c.roles.includes('all') || c.roles.includes(filteredRole)
              )
                    ...
                    {relevant.map((c, ci) => (
                      <li key={ci} className="flex items-start gap-2.5 text-sm">
                        <span className={cn(
                          ...
                          c.type === 'new'      ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400' :
                          c.type === 'improved' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400' :
        ...
        <section>
          ...
          {(filteredRole === 'employee' || filteredRole === 'manager') && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is primarily for <strong>Admins</strong> and <strong>HRBPs</strong>. Switch role filter above to Admin or HRBP to see full context.
            </div>
          )}
        ...
        <section>
          ...
          {filteredRole === 'hrbp' && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is primarily for <strong>Employees</strong>, <strong>Managers</strong>, and <strong>Admins</strong>. Switch role filter above to see full context.
            </div>
          )}
        ...
        <section>
          ...
          {(filteredRole === 'admin' || filteredRole === 'hrbp') && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is primarily for <strong>Employees</strong> and <strong>Managers</strong>. Switch role filter above to see full context.
            </div>
          )}
  ...
  Score = ((4.0 - 1) / 4) × 100 = 75%`}</CodeBlock>
        ...
        <section>
          ...
          {filteredRole === 'employee' && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is primarily for <strong>Admins</strong>, <strong>HRBPs</strong>, and <strong>Managers</strong>. Switch role filter above to see full context.
            </div>
          )}
...
Zero competency (competency_weight = 0):
  75% Goals + 25% Manager Review = 100%`}</CodeBlock>
        ...
        <section>
          ...
          {(filteredRole === 'employee' || filteredRole === 'manager') && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is primarily for <strong>Admins</strong> and <strong>HRBPs</strong>. Switch role filter above to see full context.
            </div>
          )}
...
payout_amount = snapshotted_variable_pay × rating_multiplier × proration_factor`}</CodeBlock>
        ...
        <section>
          ...
          {filteredRole !== 'admin' && (
            <div className="rounded-md border border-muted bg-muted/30 px-4 py-3 text-sm text-muted-foreground mb-4">
              This section is for <strong>Admins</strong> only. Switch role filter above to Admin to see full context.
            </div>
          )}
